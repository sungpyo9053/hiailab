...
| 2 | SNS 알림 | `LinkedIn <notifications-noreply@...>` | `기타` (※) | — |
...
### 🤖 실제로 임시보관함에 들어간 답장 초안 (Gmail Drafts API로 검증)

```
Subject: Re: 다음 주 부서 회의 일정 협의 부탁드립니다
To:      sungpyo9053@kookmin.ac.kr
---
안녕하세요. 메일 잘 받았습니다.

보내주신 일정 관련 회신드립니다. 다음 시간이 가능합니다:
- 화요일 오후 2시 ~ 4시
- 목요일 오전 10시 ~ 12시

편하신 시간 알려주시면 캘린더 초대 보내드리겠습니다.
진행 방식(대면/온라인)도 함께 알려주시면 감사하겠습니다.
...
- 이번 데모는 **분류기와 답장 본문 생성이 일시적으로 룰/하드코딩**으로 동작 중입니다. 운영 인스턴스에 등록된 OpenAI API 키의 잔액(quota)이 소진되어 OpenAI 응답이 HTTP 429 였기 때문입니다.
- **결제 수단 등록 + $5 충전** 후 코드 원복하면, classifier 가 `gpt-4o-mini` 로 본문을 보고 분류하고, 답장 본문도 본문 맥락에 맞춰 AI가 작성합니다. (gpt-4o-mini 가격대로 답장 1건 ≈ ₩1~2)
- 위 사진과 코드블록은 **실제 Gmail 임시보관함에 들어간 진짜 초안**입니다 (Drafts API `drafts.get` 으로 본문 직접 가져옴). 분류·답장 생성 로직만 일시적으로 룰/하드코딩일 뿐, Gmail OAuth + drafts.create + 임시보관함 저장은 전부 실제 동작입니다.
- (※) LinkedIn 메일이 룰에서 `기타` 로 분류된 건 룰의 한계입니다 — 정식 OpenAI 분류기로 돌리면 `알림`으로 잡힙니다.
