...
받은편지함에 5종 메일을 넣고 `/api/agent/run-once` 실행. 분류 → 답장 필요 케이스에 대해 **Gmail 임시보관함에 답장 초안 자동 생성** 까지 검증.

![처리 로그 5건 + 답장 초안 2건 임시보관함 생성](docs/images/agent-with-drafts.png)

| # | 발신자 유형 | 예시 | 분류 결과 | 임시보관함 |
| --- | --- | --- | --- | --- |
| 1 | 자동 알림 (영수증) | `Anthropic, PBC <invoice+statements@...>` | `알림` | — |
| 2 | SNS 알림 | `LinkedIn <notifications-noreply@...>` | `알림` | — |
| 3 | 사람-메일 (회의 일정 협의) | `김민재 매니저` | **`답장 필요`** | **✏️ 초안 자동 생성** |
| 4 | 사람-메일 (회의 일정 협의) | 두 번째 회의 메일 | **`답장 필요`** | **✏️ 초안 자동 생성** |
| 5 | 광고/프로모션 | `쿠팡 이벤트 — 30% 할인 쿠폰` | `광고` | — |

→ 5건 중 **답장이 필요한 2건만 정확히 골라서 임시보관함에 초안 생성**. 광고/알림/SNS는 건드리지 않음. ✓

### 🤖 실제 Gmail 임시보관함에서 본 답장 초안

위에서 분류된 답장 필요 메일에 대해 HI AI LAB 에이전트가 자동으로 만든 초안이 **본인 Gmail 임시보관함(Drafts)** 안에 그대로 들어가 있는 모습:

![Gmail 임시보관함에서 본 자동 답장 초안](docs/images/gmail-drafts.png)

> 위쪽: 받은 메일 (테스트로 보낸 회의 일정 협의 요청)
> 아래쪽: HI AI LAB이 자동 생성한 답장 초안 (`Re:` 로 시작, 본문 마지막에 "이 초안은 HI AI LAB 에이전트가 자동 작성했습니다. 발송 전 반드시 사람이 검토해 주세요." 안내 자동 부착)

자동 발송은 절대 일어나지 않습니다. 본인이 임시보관함에서 검토하고 [보내기]만 누르면 끝.


### 운영 안전장치 (코드로 검증)

- ✅ 받은편지함 자동 읽음 처리 X (`gmail.readonly` scope만 사용)
- ✅ 답장 자동 발송 X (`drafts.create`까지만 — 임시보관함)
- ✅ `OWNER_EMAIL ≠ OAuth 이메일` → 즉시 disconnect + `not_owner` 에러
- ✅ `noreply / no-reply` 발신자 메일은 분류 전 skip
- ✅ 본인 → 본인 self-메일도 자동 skip (보안)
- ✅ 메일 본문은 DB/디스크에 저장하지 않음 — 메일 ID + 분류 카테고리만 `.hiailab/processed.json` 에 기록 (중복 처리 방지용)
- ✅ 모든 비밀값은 `.hiailab/config.enc.json` 에 AES-256-GCM 암호화로 저장
...
flowchart LR
    ...
    A -->|① 5분마다 새 메일 확인<br/>읽음 처리 안 함| B
    B -->|② AI 분류<br/>답장 필요 / 뉴스레터·광고·알림 구분| B
    B -->|③ '답장 필요'만 AI가 답장 작성| C
    C -->|④ 자동 저장<br/>자동 발송 절대 안 함| D
```

### 단계 요약
1. **받은편지함 확인** — Gmail OAuth로 새 메일을 가져옴 (읽음 처리하지 않음)
2. **AI 분류** — OpenAI가 본문을 보고 `답장 필요 / 뉴스레터 / 광고 / 알림 / 스팸 / 기타` 6분류
3. **답장 초안 작성** — `답장 필요`로 분류된 메일에만 AI가 답장 본문 생성
4. **임시보관함 저장** — Gmail Drafts API로 초안 저장 (자동 발송 X)
5. **본인이 검토 후 발송** — Gmail 임시보관함에서 한 번 확인 → [보내기]
