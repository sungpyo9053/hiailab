...
5분마다 자동 폴링 + `/api/agent/run-once` 수동 트리거로 받은편지함 처리. 분류 결과:

![처리 로그 5건 — 다양한 발신 유형](docs/images/agent-with-log.png)

| 발신자 유형 | 예시 | 분류 결과 | 동작 |
| --- | --- | --- | --- |
| 자동 알림 (영수증) | `Anthropic, PBC <invoice+statements@...>` | `기타` (other) | 임시보관함 초안 X (정상) |
| 자동 알림 (SNS) | `LinkedIn <notifications-noreply@...>` | `기타` | 답장 안 만듦 (정상) |
| 광고/프로모션 | `쿠팡 이벤트` | `기타` | 답장 안 만듦 (정상) |
| 사람-메일 (외부) | _아직 테스트 필요_ | `답장 필요` (needs_reply) 예상 | **임시보관함에 초안 자동 생성** |

> ⚠ **self-메일 한계**: 본인이 본인 학교 메일로 보낸 self-메일은 분류기가 정직하게 보수적으로 `기타`로 분류합니다. 본인이 본인에게 답장을 만들어두는 게 어색하니까 의도된 동작에 가깝습니다. 진짜 `needs_reply` 데모는 **외부에서 학교 메일로 들어오는 사람-메일**이 있어야 합니다.
