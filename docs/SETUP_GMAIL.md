...
> 이 가이드는 두 가지로 나뉩니다.
> - 🟢 **쉬운 모드 (현재 지원)**: Gmail "앱 비밀번호" 로 SMTP 발송
...
본인 메일함에 **`[HI AI LAB] SMTP 연결 테스트`** 라는 제목의 메일이 도착하면 성공입니다.
...
현재 버전 범위에 **포함되지 않았습니다**. 다음 마일스톤에서 구현될 예정입니다.
...
- 앱 비밀번호는 **이 컴퓨터의 `.hiailab/config.enc.json`** 에 AES-256-GCM 암호화되어 저장됩니다.
