...
type SendKakaoBody = { title?: string; content?: string; linkUrl?: string };
...
function truncateForKakao(title: string, content: string, fallbackUrl: string) {
  ...
  if (content.length <= budget) return header + content;
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!content.trim()) {
      return NextResponse.json({ ok: false, error: "본문 내용이 비어 있습니다." }, { status: 400 });
    }
    ...
    const linkUrl = body.linkUrl?.trim() || process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000";
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        return NextResponse.json(
          { ok: false, mode: "real", error: `카카오 전송 실패 (status ${status})${kakaoMsg ? `: ${kakaoMsg}` : ""}` },
          { status: 502 }
        );
      }
      return NextResponse.json({ ok: true, mode: "real", message: "카카오톡으로 전송 완료" });
    } catch (err) {
