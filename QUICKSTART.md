# ⚡ QUICKSTART — 10분 안에 HI AI LAB 띄우기
...
4. **OpenAI API 키** — 진짜 AI 답장을 받으려면 **필수**.
   - 키 없이도 화면이 뜨고 버튼은 눌리지만, **가짜 샘플(MOCK)** 만 나옵니다.
   - 처음엔 비워두고 화면만 둘러본 뒤, 본격 사용 시작할 때 [docs/SETUP_OPENAI.md](./docs/SETUP_OPENAI.md) 따라 발급하시면 됩니다.
5. **Gmail 앱 비밀번호** — 결과를 본인 메일로 받으려면 **필수**. (없으면 "보낸 척"만 함)
6. **카카오 토큰** — 본인 카톡으로도 받고 싶으면 (선택)
...
**MOCK은 "가짜 샘플" 입니다.** 화면이 어떻게 생겼는지 분위기만 볼 수 있고, **실제 AI 답장이나 실제 메일 발송은 일어나지 않습니다.**
실제로 쓰려면 다음 단계의 `/setup` 페이지에서 키들을 채워주세요.
...
이제 본인 메일에 맞게 HI AI LAB을 사용하시거나, **[개인 서버에 올리기](./docs/DEPLOY_SERVER.md)** 가이드로 넘어가세요.
